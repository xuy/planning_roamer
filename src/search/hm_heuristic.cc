...
void HMHeuristic::init_hm_table(tuple_vector &t) {
    map<tuple_vector, int>::iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<tuple_vector, int> hm_ent = *it;
        tuple_vector &tup = hm_ent.first;
    ...
    do {
        ...
        for (int op_id = 0; op_id < g_operators.size(); op_id++) {
            ...
            if (c1 != numeric_limits<int>::max()) {
                tuple_vector eff;
                vector<tuple_vector> partial_eff;
...
void HMHeuristic::extend_tuple(tuple_vector &t, const Operator &op) {
    map<tuple_vector, int>::const_iterator it;
    for (it = hm_table.begin(); it != hm_table.end(); it++) {
        pair<tuple_vector, int> hm_ent = *it;
        tuple_vector &entry = hm_ent.first;
...
int HMHeuristic::eval(tuple_vector &t) {
    vector<tuple_vector> partial;
...
int HMHeuristic::update_hm_entry(tuple_vector &t, int val) {
...
void HMHeuristic::generate_all_tuples(int var, int sz, tuple_vector &base) {
    if (sz == 1) {
        for (int i = var; i < g_variable_domain.size(); i++) {
            for (int j = 0; j < g_variable_domain[i]; j++) {
                tuple_vector tup(base);
...
void HMHeuristic::generate_all_partial_tuple(tuple_vector &base_tuple,
                                             tuple_vector &t,
                                             int index, int sz,
                                             vector<tuple_vector> &res) {
    if (sz == 1) {
...
int HMHeuristic::check_tuple_in_tuple(const tuple_vector &tup,
                                      const tuple_vector &big_tuple) {
