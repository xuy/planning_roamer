...
class SearchSpace {
    ...
    HashTable *nodes;
    OperatorCost cost_type;
    // A list of callback functions that will be invoked whenever a node
    // is generated by using get_node.
    vector<InfoNodeCallback*> get_node_callbacks;

    // A list of callback functions that will be invoked whenever a NEW node
    // is generated by using get_node.
    vector<InfoNodeCallback*> new_node_callbacks;
...
public:
    ...
    // Takes in a callback function and iterate it through all nodes in the search space.
    void process_nodes(const InfoNodeCallback* callback);
    void add_node_callback(InfoNodeCallback* callback);
    void add_new_node_callback(InfoNodeCallback* callback);
    void invoke_callbacks(const InfoNode& node, const vector<InfoNodeCallback*>& callbacks);
};
